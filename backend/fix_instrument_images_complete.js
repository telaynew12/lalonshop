...
async function fixInstrumentImages() {
    try {
        ...
        const musicProducts = await Product.find({
            category: new mongoose.Types.ObjectId(MUSIC_CATEGORY_ID)
        });
        ...
        for (const product of musicProducts) {
            if (product.media && Array.isArray(product.media) && product.media.length > 0) {
                ...
                console.log(`⚠️  No valid media array found for: ${product.title}`);
                console.log(`   Media type: ${typeof product.media}`);
                console.log(`   Media value: ${JSON.stringify(product.media)}`);
            }
        }
